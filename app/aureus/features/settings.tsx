...
export function AddStaffModal({ gym, existingEmails, onClose, onSave }: { gym: string; existingEmails: string[]; onClose: () => void; onSave: (member: StaffMember, account: Account) => void }) {
  const [name, setName] = useState("");
  const [email, setEmail] = useState("");
  const [role, setRole] = useState<Role>("secretary");
  const [error, setError] = useState("");
  function save(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const normalizedEmail = email.trim().toLowerCase(); if (name.trim().length < 3 || !/^\S+@\S+\.\S+$/.test(normalizedEmail)) { setError("Completa un nombre y correo válidos."); return; } if (existingEmails.some((item) => item.toLowerCase() === normalizedEmail)) { setError("Ya existe una cuenta con ese correo."); return; } const id = Date.now(); const member: StaffMember = { id, name: name.trim(), email: normalizedEmail, role, status: "Invitación enviada", initials: initials(name) }; const account: Account = { id, name: name.trim(), email: normalizedEmail, password: "Aureus26", role, gym, initials: initials(name), active: true }; onSave(member, account); }
  return <ModalLayer onClose={onClose}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="staff-title" onMouseDown={(event) => event.stopPropagation()}><div className="modal__heading"><div><span>NUEVA CUENTA</span><h2 id="staff-title">Añadir personal</h2></div><button type="button" className="icon-button" onClick={onClose} aria-label="Cerrar"><X size={19} /></button></div><p>El dueño asigna el rol. La contraseña temporal de demostración será <strong>Aureus26</strong>.</p><form onSubmit={save}><label className="form-field"><span className="form-field__label">Nombre completo</span><span className="form-field__control"><UserRound size={18} /><input value={name} onChange={(event) => setName(event.target.value)} placeholder="Nombre del colaborador" autoFocus /></span></label><label className="form-field"><span className="form-field__label">Correo de acceso</span><span className="form-field__control"><Mail size={18} /><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="personal@gimnasio.com" /></span></label><label className="form-field"><span className="form-field__label">Rol y permisos</span><span className="form-field__control"><ShieldCheck size={18} /><select value={role} onChange={(event) => setRole(event.target.value as Role)}><option value="secretary">Secretaria · recepción, agenda y pagos</option><option value="trainer">Entrenador · clientes asignados</option></select></span></label>{error && <p className="form-error" role="alert">{error}</p>}<div className="modal__actions"><button type="button" className="secondary-button" onClick={onClose}>Cancelar</button><button className="primary-button primary-button--inline" type="submit"><UserPlus size={16} /> Crear invitación</button></div></form></section></ModalLayer>;
}
