...
export function RegistrationWizard({ plans, onAdd, onCancel, medical360Enabled = true, trainers = [] }: { plans: GymPlan[]; onAdd: (input: { name: string; email: string; phone: string; goal: string; plan: string; price: number; medicalDate: string; medicalTime: string; trainerId?: number }) => void; onCancel: () => void; medical360Enabled?: boolean; trainers?: Trainer[] }) {
  ...
  const selectedTrainer = trainers.find((item) => item.id === trainerId);
  ...
  function next() {
    ...
    if (step === 3 && medical360Enabled && (!medicalDate || !medicalTime)) { setError("Elige la fecha y hora de la cita médica según la disponibilidad real del médico."); return; }
    if (step === 3 && !medical360Enabled && !trainerId) { setError("Selecciona el entrenador que atenderá a este cliente."); return; }
    setStep((current) => Math.min(4, current + 1));
  }

  function finish() {
    onAdd({ name: name.trim(), email: email.trim(), phone: phone.trim(), goal, plan, price: planPrices[plan] ?? 0, medicalDate, medicalTime, trainerId: medical360Enabled ? undefined : trainerId });
  }

  return (
    <section className="registration-page">
      <div className="registration-heading"><button className="icon-button icon-button--outlined" onClick={onCancel} aria-label="Volver"><ArrowLeft size={18} /></button><div><span className="eyebrow"><UserPlus size={14} /> NUEVA PERSONA</span><h2>Registrar cliente</h2><p>{medical360Enabled ? "Registra sus datos y deja preparada la evaluación médica inicial." : "Registra sus datos y asígnale su entrenador."}</p></div></div>
      <div className="wizard-steps">{["Datos personales", "Elegir plan", medical360Enabled ? "Cita médica" : "Entrenador", "Revisar y guardar"].map((label, index) => <div className={`${step === index + 1 ? "active" : ""} ${step > index + 1 ? "done" : ""}`} key={label}><span>{step > index + 1 ? <Check size={14} /> : index + 1}</span><strong>{label}</strong><i /></div>)}</div>
      <article className="registration-card">
        ...
        {step === 2 && <div className="wizard-section"><span className="wizard-kicker">PASO 2 DE 4</span><h3>Objetivo y plan</h3><p>{medical360Enabled ? "El objetivo se tomará en cuenta junto con los resultados médicos para asignar al profesional adecuado." : "El objetivo ayuda a elegir el entrenador más adecuado en el siguiente paso."}</p><label className="form-field"><span className="form-field__label">Objetivo general</span><span className="form-field__control"><Target size={18} /><select value={goal} onChange={(event) => setGoal(event.target.value)}><option>Bienestar y hábitos activos</option><option>Fuerza y acondicionamiento</option><option>Movilidad y recuperación</option><option>Rendimiento deportivo</option></select></span></label><div className="plan-selector">{availablePlans.map((item) => <button type="button" className={plan === item.name ? "selected" : ""} key={item.id} onClick={() => setPlan(item.name)}><span>{plan === item.name && <Check size={13} />}</span><strong>{item.name}</strong><b>${item.price}<small>/mes</small></b><p>{item.description}</p></button>)}</div></div>}

        {step === 3 && medical360Enabled && <div className="wizard-section"><span className="wizard-kicker">PASO 3 DE 4</span><h3>Evaluación médica inicial</h3><p>Coordina la cita según la disponibilidad real del médico — el sistema no la asigna sola.</p><div className="wizard-form-grid"><label className="form-field"><span className="form-field__label">Fecha de la cita</span><span className="form-field__control"><CalendarDays size={18} /><input type="date" value={medicalDate} onChange={(event) => setMedicalDate(event.target.value)} /></span></label><label className="form-field"><span className="form-field__label">Hora de la cita</span><span className="form-field__control"><Stethoscope size={18} /><input type="time" value={medicalTime} onChange={(event) => setMedicalTime(event.target.value)} /></span></label></div><div className="medical-onboarding"><span><Stethoscope size={26} /></span><div><strong>Primero se evalúa, después se asigna</strong><p>El dueño registrará los resultados de los exámenes, las observaciones, recomendaciones y restricciones. Sólo entonces podrá elegir al entrenador más adecuado.</p></div></div><div className="medical-onboarding-steps"><span><b>1</b> Cita médica en agenda</span><span><b>2</b> Exámenes y observaciones</span><span><b>3</b> Asignación por el dueño</span></div></div>}

        {step === 3 && !medical360Enabled && <div className="wizard-section"><span className="wizard-kicker">PASO 3 DE 4</span><h3>Entrenador asignado</h3><p>Elige quién va a atender a este cliente desde su primer día.</p>{trainers.length ? <><div className="trainer-assignment-options" role="group" aria-label="Seleccionar entrenador">{trainers.map((trainer) => <button type="button" key={trainer.id} className={trainerId === trainer.id ? "selected" : ""} aria-pressed={trainerId === trainer.id} onClick={() => { setTrainerId(trainer.id); setError(""); }}><span className={`avatar avatar--${trainer.color}`}>{trainer.initials}</span><span><strong>{trainer.name}</strong><small>{trainer.specialty}</small><em>{trainer.openSpots} cupos · {trainer.availability}</em></span>{trainerId === trainer.id && <CheckCircle2 size={18} />}</button>)}</div>{selectedTrainer && <div className="medical-onboarding"><span><Dumbbell size={26} /></span><div><strong>{selectedTrainer.name}</strong><p>{selectedTrainer.bio || `Especialista en ${selectedTrainer.specialty.toLowerCase()}. Podrá ver a este cliente en su panel y prepararle su programa semanal.`}</p></div></div>}</> : <div className="ops-empty"><UserRound size={26} /><strong>No hay entrenadores registrados</strong><span>Pide al dueño que cree una cuenta de entrenador en la sección Equipo antes de registrar clientes.</span></div>}</div>}

        {step === 4 && <div className="wizard-section"><span className="wizard-kicker">PASO 4 DE 4</span><h3>Revisar registro</h3><p>Confirma los datos antes de añadir el cliente al gimnasio.</p><div className="registration-review"><div><span className="avatar avatar--gold">{initials(name)}</span><span><small>CLIENTE</small><strong>{name}</strong><p>{email} · {phone}</p></span></div><div><Target size={19} /><span><small>OBJETIVO</small><strong>{goal}</strong><p>Plan {plan} · ${planPrices[plan]}/mes</p></span></div>{medical360Enabled ? <div><Stethoscope size={19} /><span><small>CITA MÉDICA</small><strong>{medicalDate ? new Date(`${medicalDate}T00:00:00`).toLocaleDateString("es-PA", { day: "2-digit", month: "long" }) : ""} · {medicalTime}</strong><p>Se añadirá a la agenda privada del dueño.</p></span></div> : <div><Dumbbell size={19} /><span><small>ENTRENADOR</small><strong>{selectedTrainer?.name ?? "Sin asignar"}</strong><p>{selectedTrainer?.specialty ?? "Selecciona un entrenador en el paso anterior."}</p></span></div>}</div><div className="review-note"><ShieldCheck size={18} /><span><strong>{medical360Enabled ? "Asignación protegida" : "Listo para entrenar"}</strong><small>{medical360Enabled ? "El cliente no elige entrenador. El dueño lo asigna según los resultados médicos." : "El cliente aparecerá de inmediato en el panel de su entrenador."}</small></span></div></div>}
