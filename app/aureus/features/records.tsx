...
export function ClientRecordDetail({ client, record, role, accountName, onClose, onSave }: { client: Client; record: ClientRecord; role: Role; accountName: string; onClose: () => void; onSave: (record: ClientRecord) => void }) {
  ...
  const visibleSections = role === "secretary" ? SECTIONS.filter((item) => item.id === "health") : SECTIONS;
  ...
  function addTreatment() {
    setDraft((current) => ({
      ...
      treatments: [
        ...current.treatments,
        {
          id: current.treatments.reduce((highest, treatment) => Math.max(highest, treatment.id), 0) + 1,
          area: "Entrenamiento",
          summary: "",
          professional: "",
          status: "Activo",
    ...
    <ModalLayer className="modal-backdrop--record" onClose={onClose}>
      <section className="modal client-record-modal" role="dialog" aria-modal="true" aria-labelledby="record-title" onMouseDown={(event) => event.stopPropagation()}>
        <header className="record-heading">
          <div>
            <span>{role === "secretary" ? "FICHA ADMINISTRATIVA" : "EXPEDIENTE DEL CLIENTE"}</span>
        ...
        <div className="record-security">
          ...
          <span>
            ...
            <small>{role === "owner" ? "Incluye notas privadas y toda la información del expediente." : role === "trainer" ? "Puedes consultar antecedentes autorizados y actualizar programa y evolución." : "Los antecedentes, tratamientos y evolución clínica permanecen protegidos."}</small>
          </span>
        </div>

        <nav className="record-tabs" aria-label="Secciones del expediente">
          {visibleSections.map(({ id, label, icon: Icon }) => <button type="button" key={id} className={section === id ? "active" : ""} onClick={() => setSection(id)}><Icon size={16} /><span>{label}</span></button>)}
          {role === "secretary" && <span className="record-tabs__locked"><LockKeyhole size={14} /> Secciones clínicas protegidas</span>}
        </nav>
        ...
        <div className="record-content">
          ...
          {section === "history" && role !== "secretary" && (
            <section className="record-section">
          ...
          {section === "treatments" && role !== "secretary" && (
            <section className="record-section">
          ...
          {section === "program" && role !== "secretary" && (
            <section className="record-section">
          ...
          {section === "evolution" && role !== "secretary" && (
            <section className="record-section">
