...
export function PaymentsView({ clients, role, onMarkPaid, onSelect, onReceipt }: { clients: Client[]; role: Role; onMarkPaid: (id: number) => void; onSelect: (client: Client) => void; onReceipt?: (client: Client) => void }) {
  ...
  return (
    <section className="ops-page"><div className="ops-page__heading"><div><span className="eyebrow"><WalletCards size={14} /> PAGOS</span><h2>Revisar pagos</h2><p>{role === "secretary" ? "Confirma los pagos recibidos, entrega el recibo y prepara recordatorios por WhatsApp cuando sea necesario." : "Cuando una persona ya haya pagado, emite su recibo o pulsa “Confirmar pago”."}</p></div><button className="secondary-button" onClick={() => window.print()}><ReceiptText size={15} /> Imprimir lista</button></div><section className="payment-summary"><div><span>Registrado este mes</span><strong>${paidTotal.toFixed(2)}</strong><small>{clients.length - pending.length} mensualidades al día</small></div><div><span>Falta revisar</span><strong>{pending.length}</strong><small>Requieren seguimiento</small></div><div><span>Por cobrar</span><strong>${pending.reduce((sum, client) => sum + client.price, 0).toFixed(2)}</strong><small>De {pending.length} {pending.length === 1 ? "cliente" : "clientes"}</small></div></section><article className="panel ops-panel payments-panel"><div className="payment-table__head"><span>Persona</span><span>Plan</span><span>Fecha</span><span>Estado</span><span>Qué hacer</span></div>{clients.map((client) => <div className="payment-row" key={client.id}><button className="payment-row__client" onClick={() => onSelect(client)}><span className={`avatar avatar--${client.color}`}>{client.initials}</span><span><strong>{client.name}</strong><small>{client.phone} · {client.trainer}</small></span></button><span><strong>{client.plan}</strong><small>${client.price}/mes</small></span><span>{client.nextDue}</span><PaymentPill status={client.payment} /><div className="payment-actions">{onReceipt && <button className="payment-reminder-button" onClick={() => onReceipt(client)} aria-label={`Emitir recibo para ${client.name}`}><ReceiptText size={14} /> Recibo</button>}{client.payment === "Al día" ? <button className="paid-button" onClick={() => onSelect(client)}><Check size={14} /> Ya pagó</button> : <>{role === "secretary" && <button className="payment-reminder-button" onClick={() => sendPaymentReminder(client)} aria-label={`Preparar recordatorio de pago para ${client.name}`}><MessageCircle size={14} /> Recordar pago</button>}<button className="mark-paid-button" onClick={() => onMarkPaid(client.id)}>Confirmar pago</button></>}</div></div>)}</article></section>
  );
}
...
export function SessionEditorModal({ initial, clients, trainers, role, trainerName, onClose, onSave }: { initial: AgendaSession; clients: Client[]; trainers: Trainer[]; role: Role; trainerName: string; onClose: () => void; onSave: (session: AgendaSession) => void }) {
  const [draft, setDraft] = useState(initial);
  const [error, setError] = useState("");
...
export function AgendaView({ sessions, clients, records, trainers, role, trainerName, onSave, onMedicalComplete, medical360Enabled = true }: { sessions: AgendaSession[]; clients: Client[]; records: ClientRecord[]; trainers: Trainer[]; role: Role; trainerName: string; onSave: (session: AgendaSession) => void; onMedicalComplete: (session: AgendaSession, record: ClientRecord, trainer?: Trainer) => void; medical360Enabled?: boolean }) {
  // Sin Medical 360 no existe el proceso de evaluación médica: el dueño ve
  // la misma agenda operativa que el resto del equipo, y las citas médicas
  // que hubieran quedado de antes no se muestran.
  const medicalFlow = medical360Enabled && role === "owner";
  const permittedClients = role === "trainer" ? clients.filter((client) => client.trainer === trainerName) : medical360Enabled ? clients.filter((client) => client.trainer !== "Pendiente de evaluación médica") : clients;
  ...
  const medicalSessions = medical360Enabled ? sessions.filter((session) => session.type === "Evaluación médica").sort((a, b) => a.time.localeCompare(b.time)) : [];
  ...
  const visible = medicalFlow
    ...
    : sessions.filter((session) => session.type !== "Evaluación médica" && !session.ownerOnly && (role !== "trainer" || session.trainer === trainerName)).sort((a, b) => a.time.localeCompare(b.time));
  ...
  return (
    <section className="ops-page">
      <div className="ops-page__heading">
        <div>
          <span className="eyebrow"><CalendarDays size={14} /> {medicalFlow ? "AGENDA MÉDICA DE INGRESO" : "AGENDA DE HOY"}</span>
          <h2>{medicalFlow ? "Citas de clientes nuevos" : role === "trainer" ? "Mis sesiones" : "Citas y sesiones"}</h2>
          <p>{medicalFlow ? "Aquí aparecen automáticamente los clientes registrados por primera vez. Abre una cita para completar todo el proceso médico y asignar entrenador." : role === "secretary" ? "Pulsa una cita para verla o cambiarla." : "Sesiones programadas del gimnasio."}</p>
        </div>
        {!medicalFlow && <button type="button" className="primary-button primary-button--inline" onClick={createSession} disabled={!permittedClients.length}><Plus size={16} /> {role === "trainer" ? "Nueva sesión" : "Agregar cita"}</button>}
      </div>
      ...
      <article className="panel agenda-board">
        {medicalFlow && <div className="agenda-filter-tabs" role="group" aria-label="Filtrar citas médicas">{(["Pendientes", "Completadas", "Todas"] as const).map((filter) => <button type="button" key={filter} className={ownerFilter === filter ? "active" : ""} onClick={() => setOwnerFilter(filter)}>{filter}</button>)}</div>}
        <div className="agenda-hours">
          {visible.map((session) => {
            const client = clients.find((item) => item.id === session.clientId);
            const isMedical = medicalFlow && session.type === "Evaluación médica";
            return <button type="button" className={isMedical ? "agenda-medical-row" : ""} key={session.id} onClick={() => isMedical ? setMedicalEditing(session) : setEditing(session)}><time>{session.time}</time><i className={session.status === "Confirmada" ? "active" : ""} /><span><strong>{session.clientName}</strong><small>{isMedical && client ? `${client.goal} · Plan ${client.plan}` : `${session.focus} · ${session.status}`}</small></span><em>{isMedical ? (session.status === "Pendiente" ? "Proceso pendiente" : "Proceso completado") : session.trainer}</em>{isMedical ? <Stethoscope size={16} /> : <ChevronRight size={16} />}</button>;
          })}
          {!visible.length && <div className="agenda-empty"><CalendarCheck size={28} /><strong>{medicalFlow && ownerFilter === "Pendientes" ? "No hay evaluaciones pendientes" : "No hay citas en esta vista"}</strong><span>{medicalFlow ? "Los nuevos registros aparecerán aquí automáticamente." : "Agrega una cita para comenzar."}</span></div>}
        </div>
      </article>

      {editing && <SessionEditorModal initial={editing} clients={permittedClients} trainers={trainers} role={role} trainerName={trainerName} onClose={() => setEditing(null)} onSave={(session) => { onSave(session); setEditing(null); }} />}
      {medicalFlow && medicalEditing && medicalClient && medicalRecord && <MedicalEvaluationModal session={medicalEditing} client={medicalClient} record={medicalRecord} trainers={trainers} onClose={() => setMedicalEditing(null)} onComplete={(session, record, trainer) => { onMedicalComplete(session, record, trainer); setMedicalEditing(null); }} />}
    </section>
  );
}